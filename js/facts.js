...
const Facts = (() => {
  ...
  async function matchFacts({ home, away, sport, apiKey }) {
    ...
    try {
      ...
      // Elo des clubs : second ancrage, indépendant des buts récents
      let clubElo = null;
      if (sk === 'football' && typeof ClubElo !== 'undefined') {
        try { clubElo = await ClubElo.matchFacts({ home: ht ? ht.name : home, away: at ? at.name : away }); } catch (_) {}
      }
      ...
      const facts = {
        ...
        model, clubElo, sport: sk, source: 'api-sports'
      };
  ...
  function formatText(f) {
    ...
    if (f.clubElo && f.clubElo.text) lines.push(f.clubElo.text);
    return lines.join('\n');
  }
